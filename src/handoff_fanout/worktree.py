...
def inject_vscode_workspace(
    source_workspace: Path, wt: Path, project: str, task: str, is_coordinator: bool = False
) -> str | None:
    ...
    regression). See the ``if is_coordinator`` branch below.
    ...
    try:
        ...
        if ws_file.exists():
            # Normally respect a pre-existing (tracked/user/engine-idempotent) file & never
            # overwrite. EXCEPTION (§五·2): a coordinator MUST be red-topped even on the reuse path —
            # idempotently patch the red-top in (a pre-patch / non-coordinator-first file would else
            # open without red). A non-coordinator stays a byte-identical no-op.
            if is_coordinator:
        ...
        if is_coordinator:
            ...
            # and the singlepane fields are untouched, so a non-中枢 task is byte-identical to the
            # pre-2026-06-09 engine (validation gate #2). NOT ``ensure_ascii=False`` — that would
            # change the non-coordinator bytes too; VS Code's JSON parser decodes the 🧭中枢 \uXXXX
    ...
    *,
    ...
    if existing["exists"] or branch_exists:
        ...
        if (
            ...
            vws = inject_vscode_workspace(source_workspace, wt, project, task, is_coordinator)
            return WorktreeResult(
                status=ST_CREATED,
                spawn_workspace=wt,
                branch=br,
