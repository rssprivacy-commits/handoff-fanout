...
def main(argv: list[str] | None = None) -> int:
    ...
    sub.add_parser("dump", help="Generate handoff queue file for next task (full args: see `handoff-dump --help`)")
    sub.add_parser("watchdog", help="Run watchdog scan (fail-safe fan-in trigger; meant for launchd/cron)")
    sub.add_parser("heartbeat", help="Fan-in tab heartbeat / completion / metrics (see `handoff-heartbeat --help`)")
    sub.add_parser("safe-commit", help="Hijack-safe git commit wrapper (see `handoff-safe-commit --help`)")
