...
- 🟢〔p29+p31 已闭〕多处陈旧注释把**已上线**说成休眠：`retro_gate.py:670-672`（p29 `5b4eb20` 订正→「producer IS wired」）+ `auto-continue.sh`（p31 订正→「producer IS wired」；快照行号 2113-2116、当前 2131-2138）写 audit-overdue producer「deferred / dormant-but-ready」，但 producer 已 wired（`codex_audit.write_bypass_override`，由 `handoff audit-close` 的 codex_unavailable_bypass 模式调用 / call-site `codex_audit.py:2577`）、扫描器已跑、闸已读。原注释会误导读者把 codex bypass 当免费午餐。
...
| 6 | 文档覆盖 ⅓、陈旧注释/悬空 SOT（D）+ 本快照逐图 refresh | 🟡 中 | 非技术读者/新会话误判系统边界 | 🟢 **p30+p31 闭**：`docs/PROTOCOL.md` 扩 Part II §11-18（v5.4/audit/lock/worktree/succession=新 SOT）+ `docs/ARCHITECTURE.md` 扩 Part II（post-v1.0 层 + 指向本快照）+ 建项目级 `CLAUDE.md`；6 处源码 + 4 处 test docstring 悬空 SOT ref（`codex-audit-gate-spec-draft.md`/`v5.4-retro-mandate-draft.md`/`owner-ack-token-design.md` 三不存在文件）全改指 `docs/PROTOCOL.md`；map 01/05 的 3 处 **forward overclaim**（banner 不覆盖此类）就地订正；**p31 闭 deploy-trapped 尾巴** `auto-continue.sh:2131-2138` dormant 注释（订正→「producer IS wired」，对称 p29 的 retro_gate/codex_audit 订正·`HANDOFF_INSTALL_SH` 守 + gate 后 `--sync-launcher`）。**剩余（banner 已兜底·低价值·可延后·非阻断）**：6 子图 **inverse**-overclaim 逐行 refresh-to-HEAD（banner 已显式覆盖此类）+ 行号/LOC nits（re-drift） |
| 7 | §6c 从未 E2E 验证（A4） | 🟡 低-中 | 声称 LIVE 实为 ready-idle（实测 host_pid 写侧跑过 21×、回收 E2E 0 成功） | ⏳ 待一次真 E2E 留证（owner 在环） |
| 8 | 哨兵无界累积（B2）/ diag log 自盲（B4）/ owner_ack 非加密（A6）/ 名不副实小陷阱（A7） | 🟡 低 | 渐进/局部 | 待清理/排程/文档化 → backlog |
| 9 | mandate env 漂移无运行时自证/告警（codex p21 双脑审建议·p31 实地坐实） | 🟡 低 | env-mandate 静默丢失→静默退 legacy 无闸·无告警 | ⏳ 待定：`HANDOFF_RETRO_MANDATE`/`HANDOFF_AUDIT_MANDATE` 仅 `dump.py:280-281` 读 `== "1"`，无 drift 探测；config.json fail-closed 只护 `mandate_projects` **键**、不护 env **变量**（三路径 `.zshenv`/`launchctl setenv`/plist 任一漂移→静默退 legacy）。注：hf 本就走 legacy（不在 mandate_projects 内），其闸=pre-push+显式 evidence·env-mandate 漂移仅影响 mandate_projects 列内项目（如 erp）。低优·doc-surfaced |

**底线判断（更新至 p31 doc 尾巴）**：4 块高爆炸半径缺口 **#1（install 自毁）、#2（备份）、#3（C1 timeout）、#4（C2 spawn_lock）已全部闭环**（p28/p29·机器闸 GREEN·已部署）；**#6 文档 p30 主体 + p31 deploy-trapped 尾巴全闭**（PROTOCOL/ARCHITECTURE 扩全 + 项目 CLAUDE.md + 悬空 SOT ref 全清 + forward overclaim 订正 + auto-continue.sh dormant 注释订正→「producer IS wired」）。功能主干 live 且经测试/日志/磁盘证据支撑、可继续日常运营。**剩余（均非阻断·低价值或 owner-在环）**：#6 子图 inverse-overclaim 逐行 refresh（banner 已兜底·re-drift-prone·可不做）+ owner-在环 2 步（#5 GC --execute / #7 §6c 真 E2E）+ #8 渐进清理 + #9 mandate env-drift 自证（低优·doc-surfaced）。**距「可安全长期自托管」**：高爆炸半径项 + 文档主体与 deploy-trapped 尾巴均已清，余为 banner-兜底的文档纤维 + owner-在环验证 + 低优观测性增强。
