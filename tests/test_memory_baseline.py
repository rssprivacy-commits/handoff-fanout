...
# ─── component 4 (L2): substance floor classifier (WARN-mode shadow) ──────────

# A real-shaped lesson: ≥400B + a substance marker (根因/教训) + a concrete reference
# (file:line, git SHA, AND a [[wikilink]]) — must classify MET.
_REAL_LESSON = (
    "# lesson-demo\n\n"
    "## 根因\n"
    "watcher 计时测试在高负载下成批 fail：根因是 wall-clock deadline。\n"
    "教训：比较两次 build_evidence 全输出前必冻时间源。\n\n"
    "证据：`dump.py:1311` 改了 _retrieval_pull_warn；commit 0b90da0；详 [[lesson-sw-coord-p62]]。\n"
    + "占位说明文字，凑足最小长度门槛。" * 20
    + "\n"
)


def test_classify_substance_met_on_real_shaped_lesson(tmp_path) -> None:
    mem = tmp_path / "memory"
    mem.mkdir()
    (mem / "lesson-demo.md").write_text(_REAL_LESSON, encoding="utf-8")
    met, detail = mb.classify_substance(["lesson-demo.md"], mem)
    assert met is True, detail
    assert "lesson-demo.md" in detail


def test_classify_substance_thin_on_open_loops_only(tmp_path) -> None:
    """The Goodhart hole: touching open-loops.md / MEMORY.md with no net-new lesson ⇒ THIN."""
    mem = tmp_path / "memory"
    mem.mkdir()
    met, detail = mb.classify_substance(["open-loops.md", "MEMORY.md"], mem)
    assert met is False
    assert "no net-new lesson" in detail


def test_classify_substance_thin_on_too_short_lesson(tmp_path) -> None:
    mem = tmp_path / "memory"
    mem.mkdir()
    (mem / "lesson-stub.md").write_text("# 根因 stub [[x]] a.b:1\n", encoding="utf-8")  # < 400B
    met, detail = mb.classify_substance(["lesson-stub.md"], mem)
    assert met is False
    assert "shape floor" in detail


def test_classify_substance_thin_on_markerless_lesson(tmp_path) -> None:
    mem = tmp_path / "memory"
    mem.mkdir()
    # long + has a reference, but NO substance marker (no 坑/根因/规则/教训/lesson/why)
    body = "# notes\n" + "纯流水账没有任何洞见标记。" * 40 + "\nref a.b:1\n"
    (mem / "lesson-empty.md").write_text(body, encoding="utf-8")
    met, _ = mb.classify_substance(["lesson-empty.md"], mem)
    assert met is False


def test_classify_substance_thin_on_referenceless_lesson(tmp_path) -> None:
    mem = tmp_path / "memory"
    mem.mkdir()
    body = "# 教训\n" + "有教训二字但通篇没有任何 file:line 或 SHA 或 wikilink 引用。" * 20 + "\n"
    (mem / "lesson-noref.md").write_text(body, encoding="utf-8")
    met, _ = mb.classify_substance(["lesson-noref.md"], mem)
    assert met is False


def test_classify_substance_never_raises_on_missing_dir(tmp_path) -> None:
    """Best-effort: a missing memory dir / unreadable file degrades to THIN, never raises."""
    met, detail = mb.classify_substance(["lesson-gone.md"], tmp_path / "does-not-exist")
    assert met is False
    assert "shape floor" in detail or "error" in detail


def test_classify_substance_passes_real_p62_lesson_if_present() -> None:
    """The brief's tuning anchor: the actual p62 lesson (根因/教训/commit/file:line) must
    pass the floor (guards against over-tightening). Skipped when the file isn't on disk."""
    p62 = (
        Path.home()
        / ".claude"
        / "projects"
        / "-Users-chenmingzhong-Projects-handoff-fanout"
        / "memory"
        / "lesson-sw-coord-p62-2026-06-24.md"
    )
    if not p62.is_file():
        pytest.skip("real p62 lesson not present on this machine")
    met, detail = mb.classify_substance([p62.name], p62.parent)
    assert met is True, detail


def test_added_lessons_since_baseline_no_baseline_is_empty(tmp_path, monkeypatch, claude_root):
    """No dispatch baseline ⇒ ([], mem_dir): the substance shadow degrades silently."""
    home = _home(tmp_path, monkeypatch)
    ws = _git_repo(tmp_path)
    _memory_dir(claude_root, ws)
    added, mem_dir = mb.added_lessons_since_baseline(
        home=home, project=PROJECT, self_task="no-such-task", workspace=ws
    )
    assert added == []
    assert mem_dir == mb.memory_dir_for_workspace(ws)


def test_g3_substance_shadow_log_written_thin_on_open_loops_only(
    tmp_path, monkeypatch, claude_root, capsys
) -> None:
    """E2E: a coordinator that sediments ONLY open-loops.md (no new lesson) still passes G3
    (VERIFY_OK), prints OK, but the substance shadow logs would_block=True + WARNs (thin) —
    and rc stays 0 throughout (WARN mode never gates)."""
    home = _home(tmp_path, monkeypatch, config=json.dumps({"singlepane_projects": [PROJECT]}))
    ws = _git_repo(tmp_path)
    mem = _memory_dir(claude_root, ws)
    (mem / "MEMORY.md").write_text("index v1\n")
    ...
    # touch open-loops only — the Goodhart hole: sedimentation "proven" but no lesson.
    (mem / "open-loops.md").write_text("just a fragment\n")
    rc = codex_audit.main_audit_close(_close_argv(ws, task=NEXT_TASK, self_task=SELF_TASK))
    out = capsys.readouterr()
    assert rc == 0
    assert "OK G3-sedimentation" in out.out  # the existing line still prints
    assert "WARN G3-substance-thin" in out.err

    log = home / PROJECT / "g3-substance-shadow.log"
    assert log.is_file()
    rec = json.loads(log.read_text().splitlines()[-1])
    assert rec["substance_met"] is False
    assert rec["would_block"] is True
    assert rec["self_task"] == SELF_TASK
    assert rec["added_lessons"] == []


def test_g3_substance_shadow_log_met_on_real_lesson(
    tmp_path, monkeypatch, claude_root, capsys
) -> None:
    """E2E: a coordinator that sediments a real-shaped lesson ⇒ substance MET, would_block
    False, no thin WARN, OK line still printed, rc 0."""
    home = _home(tmp_path, monkeypatch, config=json.dumps({"singlepane_projects": [PROJECT]}))
    ws = _git_repo(tmp_path)
    mem = _memory_dir(claude_root, ws)
    (mem / "MEMORY.md").write_text("index v1\n")
    ...
    (mem / "lesson-leg7.md").write_text(_REAL_LESSON, encoding="utf-8")
    rc = codex_audit.main_audit_close(_close_argv(ws, task=NEXT_TASK, self_task=SELF_TASK))
    out = capsys.readouterr()
    assert rc == 0
    assert "OK G3-sedimentation" in out.out
    assert "WARN G3-substance-thin" not in out.err

    log = home / PROJECT / "g3-substance-shadow.log"
    rec = json.loads(log.read_text().splitlines()[-1])
    assert rec["substance_met"] is True
    assert rec["would_block"] is False
    assert "lesson-leg7.md" in rec["added_lessons"]


def test_g3_substance_no_shadow_when_no_sedimentation(
    tmp_path, monkeypatch, claude_root, capsys
) -> None:
    """The substance shadow runs ONLY on VERIFY_OK: a relay with NO sedimentation (VERIFY_WARN)
    writes no substance shadow line (it is gated behind sedimentation being proven)."""
    home = _home(tmp_path, monkeypatch, config=json.dumps({"singlepane_projects": [PROJECT]}))
    ws = _git_repo(tmp_path)
    mem = _memory_dir(claude_root, ws)
    (mem / "MEMORY.md").write_text("index v1\n")
    ...
    # NO change between dispatch and relay → VERIFY_WARN, so the substance shadow never runs.
    rc = codex_audit.main_audit_close(_close_argv(ws, task=NEXT_TASK, self_task=SELF_TASK))
    out = capsys.readouterr()
    assert rc == 0
    assert "WARN G3-no-sedimentation" in out.err
    assert not (home / PROJECT / "g3-substance-shadow.log").exists()
