...
def _setup(home: Path, tmp_path: Path, *, ui_enabled: str, keystroke_exit: int = 0) -> dict:
    ...
    # The auto-submit/accessibility path is the UNLOCKED GUI path by definition;
    # pin the lock probe so routing is deterministic on any host (incl. Linux CI
    # where ioreg is absent and would read UNKNOWN -> defer).
